...
async def process_game_turn(chat_id, user_input):
    ...
    ШКАЛА Relation_Player (15 ступенів, від найгіршого до найкращого):
    1. Смертельна ненависть → 2. Кривавий ворог → 3. Відкрита ворожість → 4. Ворожий
    → 5. Глибока підозра → 6. Підозрілий → 7. Холодний → 8. Нейтральний
    → 9. Обережно відкритий → 10. Тепле ставлення → 11. Прихильний
    → 12. Дружній → 13. Довіряє → 14. Глибока довіра → 15. Абсолютна довіра

    ПРАВИЛО ПОСТУПОВОЇ ЗМІНИ (СУВОРО):
    - За один хід Relation_Player може змінитися МАКСИМУМ на 1 ступінь шкали.
    - Стрибок через ступінь ЗАБОРОНЕНО (приклад: "Нейтральний" → "Абсолютна довіра" = ПОМИЛКА).
    - Будь-яку зміну Relation_Player ОБОВ'ЯЗКОВО поясни в полі "npc_reasoning".
    - Якщо взаємодія з NPC була нейтральною або змін у відносинах не було —
      НЕ включай Relation_Player в npc_updates взагалі.
    </field_mutation_rules>
    ...
    {{
        ...
        "npc_updates": [
            {{
                ...
                "Relation_Player": "<нове ставлення до гравця якщо змінилось, інакше ''>",
