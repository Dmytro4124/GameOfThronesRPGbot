...
def get_narrative_intro(profile):
    ...
    4. **СЮЖЕТНИЙ ГАЧОК (INCITING INCIDENT):**
       Сцена не має бути статичною ("ви стоїте"). Щось трапляється ПРЯМО ЗАРАЗ:
       - Прибув гонець з листом.
       - На вулиці почалася бійка.
       - Хтось підозрілий слідкує за героєм.
       - Герой помітив дивну деталь на бенкеті.
...
def process_game_turn(chat_id, user_input):
    ...
    try:
        ...
        # --- ТУТ ПРАЦЮЄ PYTHON-МЕНЕДЖЕР ---
        # АІ більше не торкається цифр, він лише дає "наводку"
